...
class Funcionario extends Authenticatable {
    ...
    public static function passagens_vendidas($mat_funcionario)
    {
        ...
        $qtd_vendas_hoje = DB:: select("SELECT COUNT(*) as contagem_vendas FROM venda WHERE venda.matricula_vendedor = $mat_funcionario AND venda.data_compra = '$data_hoje'");
        $qtd_vendas_7dias = DB:: select("SELECT COUNT(*) as contagem_vendas FROM venda WHERE venda.matricula_vendedor = $mat_funcionario AND venda.data_compra BETWEEN '$data_uma_semana_atras' AND '$data_hoje'");
        $qtd_vendas_30dias = DB:: select("SELECT COUNT(*) as contagem_vendas FROM venda WHERE venda.matricula_vendedor = $mat_funcionario AND venda.data_compra BETWEEN '$data_uma_mes_atras' AND '$data_hoje'");
    ...
    public static function alterarSenha($email, $novaSenha) {
        DB::update('UPDATE funcionario set password = ? where email = ?',
